...
router.post('/register', async(req,res,next)=>{
    try{
        ...
        await newUser.save();
        res.status(201).json(createResponse(true,'User registered succesfully'));
    }
    catch(err){
        next(err);
    }
...
router.post('/login', async(req,res,next)=>{
    ...
    const isMatch=await bcrypt.compare(password, user.password0);
    if(!isMatch){
        return res.status(400).json(createResponse(false,'Invalid credentials'))
    }
...
router.post('/checklogin', authTokenHandler, async(req,res,next)=>{
    res.json({
        userId: req.userId,
        ok:true,
        message: 'user authenication successful'
    })
})
